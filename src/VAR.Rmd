...
modelVAR.string <-"model {
  ...
  for (i in 2:N) {
    ...
    mu[1,i]    <- mu0[1] + alpha[1,1] * Y[1,i-1] + alpha[1,2] * Y[2,i-1]
    mu[2,i]    <- mu0[2] + alpha[2,1] * Y[1,i-1] + alpha[2,2] * Y[2,i-1]
    LogLik[i]  <- logdensity.mnorm(Y[1:2, i], mu[1:2, i], tau[1:2, 1:2])
  }
  
  # Prediction out of sample 
  ypOut[1:2, 1]  ~ dmnorm(muOut[1:2, 1], tau[1:2, 1:2])
  muOut[1,1]     <- mu0[1] + alpha[1,1] * Y[1,N] + alpha[1,2] * Y[2,N]
  muOut[2,1]     <- mu0[2] + alpha[2,1] * Y[1,N] + alpha[2,2] * Y[2,N]
  for (k in 2:Npred) {
    ypOut[1:2, k] ~ dmnorm(muOut[1:2, k], tau[1:2, 1:2])
    muOut[1,k]    <- mu0[1] + alpha[1,1] * ypOut[1,k-1] + alpha[1,2] * ypOut[2,k-1]
    muOut[2,k]    <- mu0[2] + alpha[2,1] * ypOut[1,k-1] + alpha[2,2] * ypOut[2,k-1]
  }
...
outputmcmcVAR <- jags(model.file = textConnection(modelVAR.string),
                      data = line_data,
                      parameters.to.save= c("alpha", "cov.matrix", "mu0", "Yp", "ypOut", "LogLik"), 
...
p3 <- mcmc_areas(outputmcmcVAR$samples, pars = c("alpha[1,1]"),  prob = 0.95)
p4 <- mcmc_areas(outputmcmcVAR$samples, pars = c("alpha[1,2]"),  prob = 0.95)
p5 <- mcmc_areas(outputmcmcVAR$samples, pars = c("alpha[2,1]"),  prob = 0.95)
p6 <- mcmc_areas(outputmcmcVAR$samples, pars = c("alpha[2,2]"),  prob = 0.95)
p7 <- mcmc_areas(outputmcmcVAR$samples, pars = c("cov.matrix[1,1]"),  prob = 0.95)
p8 <- mcmc_areas(outputmcmcVAR$samples, pars = c("cov.matrix[2,2]"),  prob = 0.95)
p9 <- mcmc_areas(outputmcmcVAR$samples, pars = c("cov.matrix[1,2]"),  prob = 0.95)
...
p3 <- mcmc_trace(outputmcmcVAR$samples, pars = c("alpha[1,1]"))
p4 <- mcmc_trace(outputmcmcVAR$samples, pars = c("alpha[1,2]"))
p5 <- mcmc_trace(outputmcmcVAR$samples, pars = c("alpha[2,1]"))
p6 <- mcmc_trace(outputmcmcVAR$samples, pars = c("alpha[2,2]"))
p7 <- mcmc_trace(outputmcmcVAR$samples, pars = c("cov.matrix[1,1]"))
p8 <- mcmc_trace(outputmcmcVAR$samples, pars = c("cov.matrix[2,2]"))
p9 <- mcmc_trace(outputmcmcVAR$samples, pars = c("cov.matrix[1,2]"))
